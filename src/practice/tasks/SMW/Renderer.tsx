...
export const SMWRenderer: React.FC<RendererProps> = ({ item, status, theme, onAnswerChange }) => {
  ...
  return (
    <div className="space-y-3">
      <p className="text-[10px] text-gray-400 font-mono mb-2">Choose the word that replaces the beep at the end of the recording.</p>
      {item.options?.map((opt, idx) => {
        const isSelected = selected === opt;
        return (
          <button
            key={idx}
            onClick={() => status !== 'completed' && setSelected(opt)}
