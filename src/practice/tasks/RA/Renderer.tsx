...
export const RARenderer: React.FC<RendererProps> = ({ item, status }) => {
    ...
    <div className="space-y-5">
      {item.promptText && (
        <div className="p-5 rounded-xl border border-dark-border bg-dark-surface-50">
          <p className="text-sm text-gray-200 leading-relaxed">{item.promptText}</p>
        </div>
      )}
      ...
      <div className={`rounded-2xl border p-8 text-center transition-all duration-300 ${
        ...
        <p className="text-sm font-semibold text-gray-100 mb-2">
          ...
          {!isRecording && !isSubmitted && 'Ready to record'}
        </p>

        <p className="text-xs text-gray-500">
          {isRecording && 'Your response is being recorded in real-time'}
          {isSubmitted && 'Response stored. Click Submit to continue'}
          {!isRecording && !isSubmitted && 'Recording will begin after preparation'}
        </p>
      </div>
    </div>
  );
