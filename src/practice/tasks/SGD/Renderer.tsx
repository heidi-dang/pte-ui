...
export const SGDRenderer: React.FC<RendererProps> = ({ status }) => {
    ...
    <div className="space-y-5">
      <div className={`rounded-2xl border p-8 text-center transition-all duration-300 ${
        ...
        <p className="text-sm font-semibold text-gray-100 mb-2">
          {isRecording && 'Recording... Summarize the group discussion'}
          {isSubmitted && 'Voice response captured'}
          {isPromptPlaying && 'Listen to the group discussion'}
          {!isRecording && !isSubmitted && !isPromptPlaying && 'Ready to summarize'}
        </p>

        <p className="text-xs text-gray-500">
          {isRecording && 'Cover key points, agreements, and disagreements from the discussion'}
          {isSubmitted && 'Response stored. Click Submit to continue'}
          {isPromptPlaying && 'Listen carefully to all speakers and their arguments'}
          {!isRecording && !isSubmitted && !isPromptPlaying && 'Recording begins after the discussion plays'}
        </p>
      </div>
    </div>
  );
