...
export const SGDRenderer: React.FC<RendererProps> = ({ status, theme }) => {
  return (
    <div className={`p-6 rounded-2xl border text-center ${
      ...
      <p className="text-xs font-mono font-bold tracking-widest uppercase mb-4">
        {status === 'preparing' && 'MIC STATUS: STANDBY — LISTEN TO THE DISCUSSION'}
        {status === 'recording' && 'MIC STATUS: RECORDING — SUMMARIZE THE DISCUSSION'}
        {status === 'completed' && 'MIC STATUS: CAPTURED'}
      </p>
      <span className="text-[10px] text-gray-500 font-mono">
        {status === 'preparing' && 'Listen carefully to all speakers and their arguments'}
        {status === 'recording' && 'Summarize key points, agreements, and disagreements'}
