...
export const ASQRenderer: React.FC<RendererProps> = ({ status, theme }) => {
  return (
    <div className={`p-6 rounded-2xl border text-center ${
      ...
      <p className="text-xs font-mono font-bold tracking-widest uppercase mb-4">
        {status === 'preparing' && 'MIC STATUS: STANDBY — LISTEN TO THE QUESTION'}
        {status === 'recording' && 'MIC STATUS: RECORDING — GIVE A SHORT ANSWER'}
        {status === 'completed' && 'MIC STATUS: CAPTURED'}
      </p>
      <span className="text-[10px] text-gray-500 font-mono">
        {status === 'preparing' && 'Answer with a single word or short phrase'}
        {status === 'recording' && 'Speak your answer clearly now'}
