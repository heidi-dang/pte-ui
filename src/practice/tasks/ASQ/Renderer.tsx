...
export const ASQRenderer: React.FC<RendererProps> = ({ status }) => {
    ...
    <div className="space-y-5">
      <div className={`rounded-2xl border p-8 text-center transition-all duration-300 ${
        ...
        <p className="text-sm font-semibold text-gray-100 mb-2">
          {isRecording && 'Recording... Give your short answer'}
          {isSubmitted && 'Voice response captured'}
          {isPromptPlaying && 'Listen to the question'}
          {!isRecording && !isSubmitted && !isPromptPlaying && 'Ready'}
        </p>

        <p className="text-xs text-gray-500">
          {isRecording && 'Answer with a single word or short phrase'}
          {isSubmitted && 'Response stored. Click Submit to continue'}
          {isPromptPlaying && 'A short question will be played'}
