...
export const RSRenderer: React.FC<RendererProps> = ({ status }) => {
    ...
    <div className="space-y-5">
      <div className={`rounded-2xl border p-8 text-center transition-all duration-300 ${
        ...
        <p className="text-sm font-semibold text-gray-100 mb-2">
          {isRecording && 'Recording... Repeat the sentence you heard'}
          {isSubmitted && 'Voice response captured'}
          {isPromptPlaying && 'Listen to the audio carefully'}
          {!isRecording && !isSubmitted && !isPromptPlaying && 'Ready to record'}
        </p>

        <p className="text-xs text-gray-500">
          {isRecording && 'Speak the sentence exactly as you heard it'}
          {isSubmitted && 'Response stored. Click Submit to continue'}
          {isPromptPlaying && 'You will hear the sentence once'}
          {!isRecording && !isSubmitted && !isPromptPlaying && 'Recording begins after the audio plays'}
        </p>
      </div>
    </div>
  );
