...
const AdminReliabilityPanel = ({ theme, apiFetch }: any) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {health && <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        {[{l:'Queued',v:health.queueCounts?.queued,c:'text-amber-400'},{l:'Running',v:health.queueCounts?.running,c:'text-emerald-400'},{l:'Failed',v:health.queueCounts?.failed,c:'text-red-400'},{l:'Dead Letter',v:health.queueCounts?.deadLetter,c:'text-gray-400'},{l:'Stale',v:health.staleJobs,c:'text-orange-400'}].map(k=><div key={k.l} className={`p-4 rounded-xl border text-center ${theme==='dark'?'bg-[#0f1322] border-gray-850':'bg-white border-gray-200'}`}><p className={`text-2xl font-black ${k.c}`}>{k.v}</p><p className="text-[10px] font-mono text-gray-500 uppercase">{k.l}</p></div>)}</div>}
