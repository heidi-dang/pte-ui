...
const AdminReliabilityPanel = ({ theme, apiFetch }: any) => {
  ...
  const load = async () => { setLoading(true); setError(''); try { const [h, j] = await Promise.all([apiFetch('/api/admin/runtime-health'), apiFetch(`/api/admin/jobs?status=${statusFilter}&page=${page}&pageSize=${PS}`)]); setHealth(h); setData(j || { jobs: [], total: 0 }); } catch (e: any) { setError(e.message); } finally { setLoading(false); } };
  useEffect(() => { load(); }, [apiFetch, statusFilter, page]);
